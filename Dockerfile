...
RUN git clone https://github.com/pdal/pdal && cd pdal && mkdir build  && cd build && cmake -DCMAKE_INSTALL_PREFIX=/usr -DBUILD_PLUGIN_OCI=on -DBUILD_PLUGIN_NITF=on -DBUILD_PLUGIN_P2G=on -DBUILD_PLUGIN_PCL=on -DBUILD_PLUGIN_PYTHON=on -DBUILD_PLUGIN_HEXBIN=on -DBUILD_PLUGIN_ATTRIBUTE=on -DCMAKE_BUILD_TYPE=Release -DWITH_GDAL=on -DWITH_GEOTIFF=on -DWITH_LIBXML2=on -DWITH_LASZIP=on -DWITH_LAZPERF=on  ..  && make && make install
RUN pdal info --drivers
